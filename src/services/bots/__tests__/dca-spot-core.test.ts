...
import { initialState, tick, type Snapshot, type Intent } from '@/services/bots/dca-spot/core';
...
describe('DCA_SPOT core', () => {
  ...
  it('emits PLACE_SPOT_BUY when interval has elapsed and not all orders placed', () => {
    ...
    const intent = result.intents[0] as Extract<Intent, { kind: 'PLACE_SPOT_BUY' }>;
    expect(intent.kind).toBe('PLACE_SPOT_BUY');
  ...
  it('emits BOT_DONE when ordersPlaced reaches totalOrders after a fill', () => {
    ...
    expect(kinds).toContain('PLACE_SPOT_BUY');
